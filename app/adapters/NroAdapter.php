...
class NroAdapter implements GameAdapter
{
    ...
    public function searchItems(PDO $db, string $q = '', int $limit = 50): array
    {
        ...
        try {
            $q = trim($q);
            if ($q === '') {
                $st = $db->query("SELECT id, NAME FROM item_template ORDER BY id LIMIT $limit");
            } elseif (ctype_digit($q)) {
                $st = $db->prepare("SELECT id, NAME FROM item_template WHERE id = ? OR NAME LIKE ? ORDER BY id LIMIT $limit");
                $st->execute([(int)$q, '%' . $q . '%']);
            } else {
                $st = $db->prepare("SELECT id, NAME FROM item_template WHERE NAME LIKE ? ORDER BY id LIMIT $limit");
                $st->execute(['%' . $q . '%']);
            }
            $out = [];
