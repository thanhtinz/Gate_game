    ...
    <div>
      ...
      <div class="btn-row">
        <?php foreach ($downloads as $d): ?>
          <a class="btn btn-primary" href="<?= e($d['url'] ?? '#') ?>" target="_blank" rel="noopener">⬇️ <?= e($d['label'] ?? 'Tải game') ?></a>
        <?php endforeach; ?>
        <a class="btn btn-outline" href="<?= url('/nap-xu') ?>">💰 Nạp xu</a>
        <a class="btn btn-outline" href="<?= url('/doi-xu') ?>">🔄 Đổi xu</a>
        <a class="btn btn-outline" href="<?= url('/bxh?game_id=' . $game['id']) ?>">🏆 Xếp hạng</a>
      </div>
    </div>
  </div>
</div>
...
<section class="section">
  <h2 class="section-title">🖥️ Máy chủ</h2>
  <div class="server-chips">
    <?php foreach ($servers as $s): ?>
      <span class="chip">🟢 <?= e($s['name']) ?><?= $s['note'] ? ' — ' . e($s['note']) : '' ?></span>
    <?php endforeach; ?>
  </div>
</section>
<?php endif; ?>
